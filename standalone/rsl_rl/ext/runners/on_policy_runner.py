...
from standalone.rsl_rl.ext.modules import VisionActorCritic, ActorCritic, ActorCriticRecurrent, EmpiricalNormalization
from standalone.rsl_rl.ext.algorithms import PPO, Distillation, PPOL2C2
from standalone.rsl_rl.ext.env import VecEnv
...
class OnPolicyRunner:
    ...
    def __init__(self, env: VecEnv, train_cfg, log_dir=None, device="cpu"):
        ...
        if (self.alg_cfg["class_name"]) == "PPO" or (self.alg_cfg["class_name"] == "PPOL2C2"):
        ...
        policy: ActorCritic | ActorCriticRecurrent | VisionActorCritic  = policy_class(
            num_obs, num_privileged_obs, self.env.num_actions, **self.policy_cfg
        ).to(self.device)

        # initialize algorithm
        alg_class = eval(self.alg_cfg.pop("class_name"))  # PPO, distillation
        self.alg: PPO | Distillation | PPOL2C2 = alg_class(policy, env = self.env, device=self.device, **self.alg_cfg)
