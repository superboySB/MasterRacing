...
def overwrite_python_analysis_extra_paths(isaaclab_settings: str) -> str:
    ...
    isaaclab_source_dir = os.path.join(ISAACLAB_DIR, "source")
    if os.path.exists(isaaclab_source_dir) and os.path.isdir(isaaclab_source_dir):
        source_subdirs = [d for d in os.listdir(isaaclab_source_dir) 
                         if os.path.isdir(os.path.join(isaaclab_source_dir, d))]

        rel_path = os.path.relpath(ISAACLAB_DIR, PROJECT_DIR)
        for subdir in source_subdirs:
            path_names.append(f'"${{workspaceFolder}}/{rel_path}/source/{subdir}"')
    else:
        print(f"[WARN] Could not find source directory: {isaaclab_source_dir}")


    # add the path names that are in the Isaac Lab extensions directory
    isaaclab_extensions = os.listdir(os.path.join(PROJECT_DIR, "extensions"))
    path_names.extend(['"${workspaceFolder}/extensions/' + ext + '"' for ext in isaaclab_extensions])
